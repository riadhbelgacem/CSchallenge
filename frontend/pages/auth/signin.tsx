...
export default function SignIn() {
  ...
  const handleGoogleSignIn = async () => {
    // Build the Keycloak authorization URL with kc_idp_hint parameter
    const keycloakIssuer = process.env.NEXT_PUBLIC_KEYCLOAK_ISSUER || 'http://localhost:8080/realms/resume-platform';
    const clientId = process.env.NEXT_PUBLIC_KEYCLOAK_CLIENT_ID || 'user-service';
    ...
    const authUrl = `${keycloakIssuer}/protocol/openid-connect/auth?` +
      `client_id=${clientId}&` +
    ...
    // Use NextAuth's signIn with the authorization URL approach
